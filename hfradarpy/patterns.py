...
def concat(flist):
    """
    This function takes a list of Loop objects or loop file paths and
    combines them along the time dimension using xarrays built-in concatenation
    routines.

    Args:
        loop_list (list): list of loop files or Loop objects that you want to concatenate
    ...
    for f in sorted(flist):
        if not isinstance(f, Loop):
            d = Loop(f)
        loop_list.append(d.data)

    df = pd.concat(loop_list)
    return df


class Loop(fileParser):
    """
    Loop Subclass.

    This class should be used when loading a CODAR loop (.loop) file.
    ...
    def __repr__(self):
        ...
        """
        return "<Loop: {}>".format(self.file_name)
    ...
    def to_file(self, filename, validate=True, overwrite=False):
        ...
        if not overwrite:
            if lcopy.full_file == str(filename):
        ...
        with open(filename, "w") as f:
            # Write header
            for metadata_key, metadata_value in lcopy.metadata.items():
                if "FileType" in metadata_key:
                    f.write("%% SeaSonde Crossloop Antenna Measurement File\n")
                if "ProcessingTool" in metadata_key:
                    break
                elif "ProcessedTimeStamp" in metadata_key:
                        break
            ...
            for table in lcopy._tables.keys():

                if "datetime" in lcopy._tables[table]["data"].keys():
                    lcopy._tables[table]["data"] = lcopy._tables[table]["data"].drop(["datetime"], axis=1)

                for table_key, table_value in lcopy._tables[table].items():
                    if table_key != 'data':
                        #if (table_key == 'TableType') & (table == 1):
                        #    f.write("%{}: {}\n".format(table_key, table_value))
                        #elif table_key == "TableColumns":
                            #f.write("%TableColumns: {}\n".format(len(lcopy._tables[table]["data"].columns)))
                        if table_key == "TableRows":
                            f.write("%TableRows: {}\n".format(lcopy.data.shape[0]))
                        elif table_key == "TableColumnTypes":
                            f.write("%TableColumnTypes: {}\n".format(" ".join(lcopy.data.columns.to_list())))
                ...
                if table == 1:
                    ...
                    lcopy.data = lcopy.data.fillna(999.000)
                    ...
                    row_df = pd.DataFrame([lcopy._tables[1]["_TableHeader"][0]], columns=lcopy._tables[1]["_TableHeader"][0])
                    lcopy.data.columns = lcopy._tables[1]["_TableHeader"][0]
                    if not lcopy._tables[1]['TableType'] == 'LOOP LP02':
                        row_df2 = pd.DataFrame([lcopy._tables[1]["_TableHeader"][1]],
                                               columns=lcopy._tables[1]["_TableHeader"][0])
                        lcopy.data = pd.concat([row_df, row_df2, lcopy.data], ignore_index=True)
                    else:
                        lcopy.data = pd.concat([row_df, lcopy.data], ignore_index=True)
                    lcopy.data.insert(0, "%%", np.nan)
                    lcopy.data["%%"] = lcopy.data["%%"].astype(object)# Insert column at the beginning of dataframe of NaNs
                    lcopy.data.iloc[0, lcopy.data.columns.get_loc("%%")] = "%%"  # make the first row in the first column a '%%'
                    if not lcopy._tables[1]['TableType'] == 'LOOP LP02':
                        lcopy.data.iloc[
                            1, lcopy.data.columns.get_loc("%%")] = "%%"  # make the second row in the first column a '%%'

                    # Output data table to string
                    # lcopy.data.to_string(f, index=False, justify='center', header=True, na_rep=' ')
                    lcopy.data.temp = re.sub(
                        " %%", "%%", lcopy.data.to_string(index=False, justify="right", header=False, na_rep=" ")
                    )

                    f.write(lcopy.data.temp)

                f.write("\n%TableEnd: \n")
                f.write("%%\n")

            # Write footer containing processing information
            f.write("%ProcessedTimeStamp: {}\n".format(lcopy.metadata["ProcessedTimeStamp"]))
            for tool in lcopy.metadata["ProcessingTool"]:
...
if __name__ == "__main__":
    ...
    loop_file = data_path / "patterns" / "loops" / "LISL" / "LOOP_LISL_210423_201604.loop"
    ...
    files = glob(str(data_path / "patterns" / "loops" / "LISL" / "*.loop"))

    loop_data = concat(files)
    print(f"Concatenated Data Shape: {loop_data.shape}")
